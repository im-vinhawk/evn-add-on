...
from .api import EvnApiError, EvnAuthenticationError, EvnClient
from .calculation import aggregate_bills, aggregate_daily, aggregate_overviews
from .const import (
    CONF_ACCESS_TOKEN, CONF_CURRENT_CUSTOMER_CODE, CONF_CUSTOMER_CODES, CONF_DEVICE_ID, CONF_PRIMARY_CUSTOMER_CODE,
...
def configured_customer_codes(entry: ConfigEntry) -> list[str]:
    ...
    raw_codes = [entry.data.get(CONF_PRIMARY_CUSTOMER_CODE, ""), *entry.options.get(CONF_CUSTOMER_CODES, [])]
    codes: list[str] = []
...
class EvnDataUpdateCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        self._client = EvnClient(async_get_clientsession(hass), self._state)
        ...
        async with self._update_lock:
            ...
            for code in codes:
                try:
                    ...
                    _LOGGER.debug("EVN update failed for a configured customer: %s", err)
    ...
    def _persist_changed_tokens(self) -> None:
        ...
        updated = {**self.config_entry.data, **self._state.as_dict()}
        for key in (CONF_USERNAME, CONF_ACCESS_TOKEN, CONF_REFRESH_TOKEN, CONF_DEVICE_ID, CONF_PRIMARY_CUSTOMER_CODE, CONF_CURRENT_CUSTOMER_CODE):
